        ...
        <table style="width: 100%;" class="table-no-border">
          ...
          <tbody>
                ...
                @foreach($voucherRoom as $room_id => $hRoom)
                     @php
                // Hitung total biaya weekday
                $totalWeekDay = $rooms[$room_id]->weekday_price * $hRoom['count'] * $weekDay;

                // Hitung total biaya weekend
                $totalWeekEnd = $rooms[$room_id]->weekend_price * $hRoom['count'] * $weekEnd;

                // Total biaya kamar ini (weekday + weekend)
                $roomTotal = $totalWeekDay + $totalWeekEnd;
            ...
            @endphp
                ...
                <tr>
                   ...
                   <td class="text-right"><span style="float: left">Rp </span><span style="float: right">{{ number_format($total) }}</span></td>
